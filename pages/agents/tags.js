...
const Tags = () => {
  ...
  const { data: amelTags, isLoading: isLoadingTags, mutate: mutateTags, error: errorTags } = useSWR(`label?page=${page}&search=${searchQuery}`, fetcherGetMission)
  const { trigger: deleteTags, isMutating: isDeletingTags } = useMutation(`label`, createDelete, { onSuccess: () => mutateTags() })



  useEffect(() => {
    console.log(amelTags?.message, 'ameltags')
  }, [amelTags])
            ...
            <div class="sm:flex sm:items-center py-2 sm:justify-between">
              <div>
                <div class="flex items-center gap-x-3">
                  <h2 class="text-lg font-medium text-gray-800 ">برچسب ها</h2>

                  <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">240 برچسب</span>
                </div>
                  ...
                  <table class="min-w-full divide-y divide-gray-200  overflow-scroll">
                    <thead class="bg-gray-50 " style={{ direction: 'rtl' }}>
                      <tr>
                        ...
                        <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 text-right">مقدار</th>

                        <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500  text-right">شناسه</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 " style={{ direction: 'rtl' }}>
                      ...
                      {
                        amelTags?.message?.map((tag) => (
                          <tr>
                            <td class="px-12 py-4 text-sm font-medium whitespace-nowrap">
                              <div class="flex items-center mb-4">
                                <h2 class="font-medium text-gray-800 dark:text-white ">{tag?._source?.title}</h2>

                              </div>
                            </td>
                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                              <div>
                                <div style={{ backgroundColor: `${tag?._source?.color}` }} class={`object-cover w-6 h-6 -mx-1   rounded-full dark:border-gray-700 shrink-0`} alt="" ></div>
                            ...
                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                            <div class="inline px-3 py-1 text-sm font-normal text-white bg-blue-500 text-right rounded-full m-1  gap-x-2 dark:bg-gray-800">                                {tag?._source?.value}
                              </div>
                            </td>
                            ...
                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                              ...
                              <div class="inline px-3 py-1 text-sm font-normal text-white bg-blue-500 text-right rounded-full m-1  gap-x-2 dark:bg-gray-800">                                  {tag?._source?.category + '_' + tag?._source?.value}
                                </div>
                            ...
                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                              <div class='flex flex-row'>
                                <svg style={{ cursor: 'pointer' }} onClick={() => deleteTags({ id: tag?._id })} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-5 h-5">
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                </svg>
                                <a href={`/lists/edit_tag?id=${ tag?._id }`}>
...
export default withLayout(<PanelLayout />)(Tags)
